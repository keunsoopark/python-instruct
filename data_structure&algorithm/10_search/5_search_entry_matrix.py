def find_elem_matrix_bool_kp(m1, value):
    for row in m1:
        if value < min(row) or value > max(row):
            continue
        if value in row:
            return True
...
def test_find_elem_matrix_bool():
    ...
    assert(find_elem_matrix_bool_kp(m1, 8) is True)
    assert(find_elem_matrix_bool_kp(m1, 3) is False)
    m2 = [[0]]
    assert(find_elem_matrix_bool_kp(m2, 0) is True)
